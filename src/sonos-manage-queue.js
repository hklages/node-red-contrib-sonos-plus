...
module.exports = function(RED) {
  ...
  function insertSpotifyUri(node, msg, sonosPlayer) {
    ...
    if (
      ...
      failure(
        ...
        msg,
        new Error(
          'n-r-c-s-p: topic must be track, album, artistTopTracks or playlist'
        ),
        sonosFunction
      );
      return;
    ...
    if (!isTruthyAndNotEmptyString(msg.region)) {
      ...
      if (msg.region.match(regex)) {
        ...
        failure(
          node,
          msg,
          new Error('n-r-c-s-p: invalid region specified - must be 4 digits'),
          sonosFunction
        );
        return;
      }
  ...
  function insertPrimePlaylistUri(node, msg, sonosPlayer) {
    ...
    if (!isTruthyAndNotEmptyString(msg.topic)) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: undefined prime playlist'),
        sonosFunction
      );
      return;
    }
    if (!msg.topic.startsWith('x-rincon-cpcontainer:')) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: invalid prime playlist'),
        sonosFunction
      );
      return;
    }
  ...
  function insertMySonosSpotify(node, msg, sonosPlayer, onlyPlaylists) {
    ...
    sonosPlayer
      ...
      .then(response => {
        ...
        if (response === false) {
          throw new Error(
            'n-r-c-s-p: Could not find any My Sonos items or player not reachable'
          );
        }
        if (
        ...
        for (let i = 0; i < parseInt(response.items.length); i++) {
          if (
            typeof response.items[i].uri === 'undefined' ||
            response.items[i].uri === null ||
            (typeof response.items[i].uri === 'number' &&
              isNaN(response.items[i].uri)) ||
            response.items[i].uri === ''
          ) {
            warning(
              node,
              sonosFunction,
              'item does NOT have uri property',
              'item does NOT have uri property - ignored'
            );
          } else {
            ...
            if (playlistUri.indexOf(SERVICE_IDENTIFIER) > 0) {
              ...
              playlistUri = response.items[i].uri;
              if (
                typeof response.items[i].title === 'undefined' ||
                response.items[i].title === null ||
                (typeof response.items[i].title === 'number' &&
                  isNaN(response.items[i].title)) ||
                response.items[i].title === ''
              ) {
                warning(
                  node,
                  sonosFunction,
                  'item does NOT have Title property',
                  'item does NOT have Title property - ignored'
                );
                itemTitle = 'unknown';
              } else {
      ...
      .then(playlistArray => {
        ...
        if (position === -1) {
          throw new Error(
            'n-r-c-s-p: could not find playlist name in playlists'
          );
        } else {
      ...
      .then(uri => {
        ...
        if (spotifySplit.length < 2) {
          throw new Error(
            'n-r-c-s-p: invalid uri syntax: %3a' + JSON.stringify(uri)
          );
        }
        ...
        if (spotifySplit.length < 0) {
          throw new Error(
            'n-r-c-s-p: invalid uri syntax - ?: ' + JSON.stringify(uri)
          );
        }
  ...
  function insertMySonosAmazonPrimePlaylist(node, msg, sonosPlayer) {
    ...
    sonosPlayer
      ...
      .then(response => {
        ...
        if (playlistArray.length === 0) {
          throw new Error(
            'n-r-c-s-p: could not find any amazon prime playlist'
          );
        }
        return playlistArray;
      })
  ...
  function insertSonosPlaylist(node, msg, sonosPlayer) {
      ...
      if (Number.isInteger(listDimension)) {
        if (listDimension > 0) {
          ...
          failure(
            node,
            msg,
            new Error('n-r-c-s-p: msg.size is not positve: ' + msg.size),
            sonosFunction
          );
          return;
        }
        ...
        failure(
          node,
          msg,
          new Error('n-r-c-s-p: msg.size is not an integer: ' + msg.size),
          sonosFunction
        );
        return;
      }
    ...
    sonosPlayer
      .getMusicLibrary('sonos_playlists', { start: 0, total: listDimension })
      .then(response => {
        // validate response
        if (!isTruthyAndNotEmptyString(response)) {
        ...
        if (response === false) {
          throw new Error(
            'n-r-c-s-p: Could not find any playlists or player not reachable'
          );
        }
        if (
          typeof response.items === 'undefined' ||
        ...
        if (playlistArray.length === listDimension) {
          warning(
            node,
            sonosFunction,
            'There may be more playlists.',
            'Please use/modify msg.size'
          );
        }
        return playlistArray;
      })
      .then(playlistArray => {
        ...
        if (position === -1) {
          ...
          node.debug(
            'founde uri: ' + JSON.stringify(playlistArray[position].uri)
          );
          return playlistArray[position].uri;
        }
      })
  ...
  function insertMusicLibraryPlaylist(node, msg, sonosPlayer) {
    ...
    sonosPlayer
      .getMusicLibrary('playlists', { start: 0, total: listDimension })
      .then(response => {
        // get array of playlists and return
        if (!isTruthyAndNotEmptyString(response)) {
          throw new Error(
            'n-r-c-s-p: undefined getMusicLibrary response received'
          );
        }
        if (response === false) {
          throw new Error(
        ...
        if (isTruthyAndNotEmptyString(msg.volume)) {
          ...
          if (Number.isInteger(newVolume)) {
            if (newVolume > 0 && newVolume < 100) {
              ...
              throw new Error(
                'n-r-c-s-p: msg.volume is out of range 1...99: ' + newVolume
              );
            }
          } else {
            node.debug('msg.volume is not number');
            throw new Error(
              'n-r-c-s-p: msg.volume is not a number: ' +
      ...
      .then(queueSize => {
        ...
        if (position === 'last') {
          ...
          if (position < 1 || position > queueSize) {
            throw new Error(
              'n-r-c-s-p: index (msg.topic) is out of range: ' +
                String(position)
            );
          }
        }
  ...
  function removeSongFromQueue(node, msg, sonosPlayer) {
    ...
    sonosPlayer
      ...
      .then(queueSize => {
        ...
        if (numberOfSongs < 1) {
          throw new Error(
            'n-r-c-s-p: numberOfSongs is out of range - less than 1'
          );
        }
      ...
      .then(() => {
        return sonosPlayer.removeTracksFromQueue(
          validatedPosition,
          validatedNumberofSongs
        );
      })
      .then(response => {
  ...
  function setQueuemode(node, msg, sonosPlayer) {
    ...
    if (playmodes.indexOf(msg.topic) === -1) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: this topic is not allowed ' + msg.topic),
        sonosFunction
      );
      return;
    }
    ...
    sonosPlayer
      ...
      .then(mediaInfo => {
        ...
        if (
          typeof mediaInfo.CurrentURI === 'undefined' ||
          mediaInfo.CurrentURI === null ||
          (typeof mediaInfo.CurrentURI === 'number' &&
            isNaN(mediaInfo.CurrentURI)) ||
      ...
      .then(response => {
        ...
        if (response === false) {
          ...
          songsArray.forEach(function(songsArray) {
            if (
              typeof songsArray.albumArtURL === 'undefined' ||
              songsArray.albumArtURL === null ||
              (typeof songsArray.albumArtURL === 'number' &&
                isNaN(songsArray.albumArtURL)) ||
              songsArray.albumArtURL === ''
            ) {
              // ignore this item
              node.debug(
                'albumArtURL not available' + JSON.stringify(songsArray)
              );
            } else {
              songsArray.albumArtURI = songsArray.albumArtURL;
              songsArray.albumArtURL =
                sonosPlayer.baseUrl + songsArray.albumArtURI;
            }
          });
        }
  ...
  function seek(node, msg, sonosPlayer) {
    ...
    if (!REGEX_TIME.test(msg.topic)) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: msg.topic must have format hh:mm:ss, hh < 20'),
        sonosFunction
      );
      return;
    }
