  ...
  */
  function processInputMsg (node, msg, ipaddress) {
    ...
    if (command === 'play_mysonos') {
      ...
      helper.showWarning(node, sonosFunction, 'command depreciated', 'please use similar command in get status node');
  ...
  */
  function playTuneIn (node, msg, sonosPlayer) {
    ...
    if (reg.test(msg.topic)) {
      ...
      helper.showErrorMsg(node, msg, new Error('n-r-c-s-p: invalid TuneIn radio id: ' + JSON.stringify(msg.topic)), sonosFunction);
    }
  }

  /**  Play a specific My Sonos station (only TuneIn, AmazonPrime, MP3 station), start playing and optionally set volume.
  * @param  {Object} node current node
  * @param  {object} msg incoming message
                topic search string
        ...
        for (let i = 0; i < stations.length; i++) {
          if (((stations[i].title).indexOf(msg.topic)) >= 0) {
            ...
            if (stations[i].source === 'TuneIn') {
              ...
              return sonosPlayer.playTuneinRadio(stations[i].radioId);
            } else if (stations[i].source === 'AmazonPrime') {
              stationTitleFinal = stations[i].title;
              return sonosPlayer.setAVTransportURI(stations[i].uri);
            } else if (stations[i].source === 'MP3Stream') {
              stationTitleFinal = stations[i].title;
              return sonosPlayer.setAVTransportURI(stations[i].uri);
            } else {
