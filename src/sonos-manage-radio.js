  ...
  function playTuneIn(node, msg, sonosPlayer) {
    ...
    if (!isTruthyAndNotEmptyString(msg.topic)) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: undefined TuneIn id'),
        sonosFunction
      );
      return;
    }
    ...
    if (REGEX_RADIO_ID.test(msg.topic)) {
      ...
      failure(
        ...
        msg,
        new Error(
          'n-r-c-s-p: invalid TuneIn radio id: ' + JSON.stringify(msg.topic)
        ),
        sonosFunction
      );
    }
  ...
  function playHttpRadio(node, msg, sonosPlayer) {
    ...
    if (!isTruthyAndNotEmptyString(msg.topic)) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: undefined topic', sonosFunction)
      );
      return;
    }
    ...
    if (!msg.topic.startsWith('http')) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: topic should start with http', sonosFunction)
      );
      return;
    }

    sonosPlayer
      .setAVTransportURI(msg.topic)
      .then(() => {
        // optionally change volume
        // validate volume: integer, betweent 1 and 99
        if (isTruthyAndNotEmptyString(msg.volume)) {
      ...
      .then(response => {
        ...
        for (let i = 0; i < response.items.length; i++) {
          if (
            ...
            node.debug(
              'uri not define - this record is ignored' +
                String(i) +
            ...
            if (
              ...
              throw new Error(
                'n-r-c-s-p: undefined title at position ' + String(i)
              );
            }
        ...
        if (stationArray.length === 0) {
          throw new Error(
            'n-r-c-s-p: no TuneIn/Amazon/MP3Radio station in My Sonos'
          );
        }
        if (ingnoredItems > 0) {
          warning(
            node,
            sonosFunction,
            'Some My Sonos items do not contain an uri',
            'Count: ' + String(ingnoredItems)
          );
        }
        ...
        for (let i = 0; i < stations.length; i++) {
          if (stations[i].title.indexOf(msg.topic) >= 0) {
            ...
            if (
              ...
              throw new Error(
                'n-r-c-s-p: Neither tuneIn nor amazon nor mp3 radio'
              );
            }
          }
        }
        ...
        for (let i = 0; i < response.items.length; i++) {
          if (
            ...
            if (stationUri.startsWith(TUNEIN_PREFIX)) {
              ...
              let stationLogo;
              if (
                typeof response.items[i].albumArtURI === 'undefined' ||
                response.items[i].albumArtURI === null ||
                (typeof response.items[i].albumArtURI === 'number' &&
                  isNaN(response.items[i].albumArtURI)) ||
                response.items[i].albumArtURI === ''
              ) {
                stationLogo = '';
              } else {
