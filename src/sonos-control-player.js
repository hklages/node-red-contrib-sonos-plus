        ...
        sonosPlayer
          ...
          .then(zoneData => {
            ...
            if (!isTruthy(zoneData.CurrentZoneGroupName)) {
              throw new Error(
                'n-r-c-s-p: undefined CurrentZoneGroupName received'
              );
            }
            if (zoneData.CurrentZoneGroupName === '') {
              throw new Error(
                'n-r-c-s-p: player is in client mode - command rejected'
              );
            }
            return true;
          })
        ...
        if (Number.isInteger(volumeValue)) {
          if (volumeValue > 0 && volumeValue < 100) {
            ...
            failure(
              ...
              msg,
              new Error(
                'n-r-c-s-p: volume is out of range: ' + String(volumeValue)
              )
            );
            return;
          }
          ...
          failure(
            node,
            msg,
            new Error('n-r-c-s-p: volume is not valid number: ' + volumeValue)
          );
          return;
        }
        sonosPlayer
  ...
  function handleSetLed(node, msg, sonosPlayer) {
    ...
    if (!(msg.topic === 'On' || msg.topic === 'Off')) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: topic must be On or Off'),
        sonosFunction
      );
      return;
    }
  ...
  function setEQ(node, msg, sonosPlayer) {
    ...
    if (!isTruthyAndNotEmptyString(msg.eqvalue)) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: undefined new value'),
        sonosFunction
      );
      return;
    }
    ...
    if (eqType === 'SubGain') {
      ...
      if (Number.isInteger(msg.eqvalue)) {
        if (msg.eqvalue < -15 || msg.eqvalue > 15) {
          failure(
            node,
            msg,
            new Error('n-r-c-s-p: msg.eqvalue must be in range -15 to +15'),
            sonosFunction
          );
          return;
        }
        ...
        failure(
          node,
          msg,
          new Error('n-r-c-s-p: msg.eqvalue must be of type integer'),
          sonosFunction
        );
        return;
      }
      ...
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: EQType in msg.topic is not yet supported'),
        sonosFunction
      );
      return;
    }
  ...
  function createStereoPair(node, msg, sonosPlayer) {
    ...
    if (!isTruthyAndNotEmptyString(msg.topic)) {
      failure(
        node,
        msg,
        new Error('n-r-c-s-p: undefined msg.topic', sonosFunction)
      );
      return;
    }
    sonosPlayer
